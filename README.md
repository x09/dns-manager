...
- Добавлено множественное удаление записей. Правая панель записей теперь поддерживает Shift (диапазон) и Ctrl (отдельные записи). При удалении из выбора автоматически отбрасываются папки и нередактируемые записи (`NS`, `SOA`) — удаляются только обычные записи.
